...
describe('reactivity/readonly', () => {
  ...
  describe('Object', () => {
    /*
        这个测试太多了，不写了，源码中如果lock锁没有锁上
        readonly对象的行为和reactive对象的行为一致
        readonly禁止了会有影响的操作
    */
  ...
  describe('Array', () => {
    ...
    it('should not allow mutation', () => {
      ...
      // 变异方法在内部调用set / length，因此也被阻塞
      observed.push(2)
      expect(observed.length).toBe(1)
      // 推送会在[1]和.length上触发两个警告
  ...
  //在readonly调用reactive应该返回readonly
  ...
  //在reactive调用readonly应该返回readonly
